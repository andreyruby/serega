...
```ruby
# allow_missing_key: nil instead of raising KeyError
class UserSerializer < Serega
...
UserSerializer.to_h({})
# => {middle_name: nil, status: "active"}
```

```ruby
# config.hash_access.default_mode / default_allow_missing_key set the defaults for every attribute
class UserSerializer < Serega
